...
class ExperimentConfig:
    ...
    # Expects a list of 46 floats. Useful for upweighting hard landmarks (ears).
...
EXPERIMENT_PRESETS: dict[str, ExperimentConfig] = {
    ...
    # --- Pure heatmap supervision WITHOUT mixup (clean test of heatmap loss) ---
    "pure_heatmap_clean": ExperimentConfig(
    ...
    # --- Pure heatmap with larger sigma (DeepPoseKit-style) ---
    "pure_heatmap_s35": ExperimentConfig(
        name="pure_heatmap_s35",
        backbone="efficientnetv2s",
        head_type="heatmap",
        heatmap_dropout=0.1,
        num_deconv_layers=4,
    ...
    # --- Stronger augmentation with coord loss ---
    "strong_aug": ExperimentConfig(
        name="strong_aug",
        backbone="efficientnetv2s",
        head_type="heatmap",
        heatmap_dropout=0.1,
        ...
        crop_margin=0.10,
        aug_rotation=True,
        aug_rotation_deg=30.0,
        aug_flip=True,
        aug_crop_jitter=True,
        aug_crop_jitter_frac=0.12,
        aug_scale=True,
    ...
    # --- Pure heatmap + strong augmentation ---
    "pure_heatmap_strong_aug": ExperimentConfig(
        name="pure_heatmap_strong_aug",
        backbone="efficientnetv2s",
        head_type="heatmap",
        heatmap_dropout=0.1,
        num_deconv_layers=4,
    ...
    # --- Moderate augmentation (between tight_margin 15° and strong_aug 30°) ---
    "moderate_aug": ExperimentConfig(
        name="moderate_aug",
        backbone="efficientnetv2s",
        head_type="heatmap",
        heatmap_dropout=0.1,
    ...
    # --- Coord loss with higher SoftArgmax beta during training ---
    "tight_margin_beta10": ExperimentConfig(
    ...
    # --- 256x256 + beta=10 (combining two improvements) ---
    "tight_margin_256_beta10": ExperimentConfig(
        name="tight_margin_256_beta10",
        backbone="efficientnetv2s",
        head_type="heatmap",
        heatmap_dropout=0.1,
        num_deconv_layers=4,
    ...
    # --- 320x320 resolution (if 256 helped, bigger may help more) ---
    ...
    # --- DenseNet121 + SoftArgmax2D coord loss (proven approach, DenseNet backbone) ---
    "densenet121_coord_224": ExperimentConfig(
        name="densenet121_coord_224",
        backbone="densenet121",
        head_type="heatmap",
        heatmap_dropout=0.1,
        num_deconv_layers=4,  # 7x7 -> 112x112 heatmaps
        epochs=100,
        finetune_epochs=200,
        finetune_learning_rate=1e-5,
        finetune_last_layers=50,
    ...
    # --- DenseNet121 + SoftArgmax2D coord loss at 320px ---
    "densenet121_coord_320": ExperimentConfig(
        name="densenet121_coord_320",
        backbone="densenet121",
        head_type="heatmap",
        heatmap_dropout=0.1,
        num_deconv_layers=4,  # 10x10 -> 160x160 heatmaps
        epochs=100,
        finetune_epochs=200,
        finetune_learning_rate=1e-5,
        finetune_last_layers=50,
    ...
    # --- DenseNet121 DLC-style: 4 deconv (112x112), per-landmark heatmap, sigma=3 ---
    "dlc_densenet121_224": ExperimentConfig(
        name="dlc_densenet121_224",
        backbone="densenet121",
        head_type="heatmap",
        heatmap_dropout=0.1,
        num_deconv_layers=4,  # 7x7 -> 112x112 heatmaps
    ...
    # --- DenseNet121 DLC-style at 320px: 4 deconv (160x160), sigma=4 ---
    "dlc_densenet121_320": ExperimentConfig(
        name="dlc_densenet121_320",
        backbone="densenet121",
        head_type="heatmap",
        heatmap_dropout=0.1,
        num_deconv_layers=4,  # 10x10 -> 160x160 heatmaps
    ...
    # --- 256x256 with 2x ear landmark weighting ---
    # Per-landmark analysis: ears NME ~14 vs eyes ~5. Upweight ears 2x in loss.
    # Landmarks 0-8 = right ear, 9-17 = left ear (18 ear landmarks total).
    "tight_margin_256_ear_weight": ExperimentConfig(
        name="tight_margin_256_ear_weight",
        backbone="efficientnetv2s",
        head_type="heatmap",
        heatmap_dropout=0.1,
        num_deconv_layers=4,
        ...
        lm_margin=0.05,
        crop_margin=0.10,
        landmark_weights=[2.0]*18 + [1.0]*28,  # 2x for ears (0-17), 1x for rest
        aug_rotation=True,
        aug_rotation_deg=15.0,
        aug_flip=True,
        aug_crop_jitter=True,
    ...
    # --- 256x256 with higher dropout to fight overfitting (gap=3.5) ---
    "tight_margin_256_highdrop": ExperimentConfig(
        name="tight_margin_256_highdrop",
        backbone="efficientnetv2s",
        head_type="heatmap",
        heatmap_dropout=0.25,  # up from 0.1
        num_deconv_layers=4,
        epochs=100,
        finetune_epochs=200,
        finetune_learning_rate=1e-5,
        ...
        loss="mse",
        optimizer="adamw",
        weight_decay=3e-4,  # up from 1e-4
        use_swa=False,
        img_size=256,
        lm_margin=0.05,
        crop_margin=0.10,
    ...
    # --- Pure heatmap with LARGE sigma (sigma=10 in heatmap space) ---
    "pure_heatmap_s10": ExperimentConfig(
        name="pure_heatmap_s10",
        backbone="efficientnetv2s",
        head_type="heatmap",
        heatmap_dropout=0.1,
        num_deconv_layers=4,
...
def _make_weighted_mse(weights_list: list):
    ...
    weights_list: 46 floats, one per landmark. Internally normalized so
    sum(weights) == 46 (preserving same loss scale as unweighted MSE).
    ...
    # Repeat each weight for x,y -> shape [92]
    w_flat = np.repeat(w, 2).astype(np.float32)
    w_tensor = tf.constant(w_flat, dtype=tf.float32)  # [92]

    @tf.keras.utils.register_keras_serializable(package="DogFLW")
    def weighted_mse_fn(y_true, y_pred):
        sq_diff = tf.square(y_true - y_pred)  # [B, 92]
        weighted = sq_diff * w_tensor  # [B, 92]
