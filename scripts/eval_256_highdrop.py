#!/usr/bin/env python3
"""Multi-scale TTA evaluation for the tight_margin_256_highdrop model.
...
Usage:
    PYTHONUNBUFFERED=1 python scripts/eval_256_highdrop.py
"""
...
MODEL_PATH = Path("artifacts/tight_margin_256_highdrop/best.keras")
...
def main():
    ...
    cfg = copy.deepcopy(EXPERIMENT_PRESETS["tight_margin_256_highdrop"])
    print(f"Preset:      tight_margin_256_highdrop")
    print(f"img_size:    {cfg.img_size}")
    print(f"crop_margin: {cfg.crop_margin}")
    print(f"lm_margin:   {cfg.lm_margin}")
    print(f"heatmap_dropout: {cfg.heatmap_dropout}")
    ...
    print()
    print("=" * 70)
    print("RESULTS — tight_margin_256_highdrop multi-scale TTA")
    print("=" * 70)
    ...
    print(f"Reference (256 tight_margin):   TBD")
