...
python > /tmp/rtmpose_onnx2tf_fixed.log 2>&1 << 'PYEOF'
...
print('Starting conversion on GAU-fixed ONNX...', flush=True)
onnx2tf.convert(
    input_onnx_file_path='checkpoints/rtmpose_s_gau_fixed.onnx',
...
echo "exit code: $?" >> /tmp/rtmpose_onnx2tf_fixed.log
